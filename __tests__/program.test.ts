...
jest.mock("../models/program.model", () => ({
  fetchPrograms: jest.fn(() => Promise.resolve(programs)),
}));

describe("GET /api/programs", () => {
  test("should return a list of all programs", async () => {
    const response = await request(app).get("/api/programs");

    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty("programs");
    expect(response.body.programs).toEqual(programs);
  });
});

describe('POST /api/programs', () => {
    test('should add a new program', async () => {
      const newProgram = {
        id: 12,
        title: 'New Program',
        topic: 'test-topic',
      ...
      expect(response.body.program).toEqual(newProgram);
    });
  });